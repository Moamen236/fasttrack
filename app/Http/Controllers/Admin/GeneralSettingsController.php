...
class GeneralSettingsController extends Controller
{
    ...
    public function update_license_settings(Request $request)
    {
        ...
        $response = Http::post('https://bytesed.com/api/license/new', [
            ...
            'site_url' => url('/'),
            'item_unique_key' => '92RYWPV0DAowHzq2SsRS16NiFTcsWceJ',
        ]);
        $result = $response->json();
        $msg = __("something went wrong, contact support");
        $type = 'danger';
        if($response->ok() ){
			*/
		$this->validate($request, [
            'item_purchase_key' => 'required'
        ]);
      
        $result = array();
        $result['license_status'] = 'verified';
        $result['msg'] = 'Verified successfully!';
        ...
        if(true){
            ...
            $license_info = [
                ...
                "xgenious_app_key" => '92RYWPV0DAowHzq2SsRS16NiFTcsWceJ',
